...
fn claim_next_hands_out_the_oldest_task_first() {
    ...
    let claim = queue
        .claim_next(&mut store, &leases, &worker(1))
        .unwrap()
        .unwrap();
...
fn each_worker_gets_a_different_task() {
    ...
    let a = queue
        ...
        .unwrap();
    let b = queue
        .claim_next(&mut store, &leases, &worker(2))
        .unwrap()
        .unwrap();
    let c = queue
        .claim_next(&mut store, &leases, &worker(3))
        .unwrap()
        .unwrap();
...
fn failure_within_budget_schedules_a_retry() {
    ...
    queue
        ...
        .unwrap();
    store
        .transition(&ids[0], TaskState::Running, None, None, clock.now())
        .unwrap();

    let verdict = queue
        .record_failure(&mut store, &ids[0], "script exited 1")
        .unwrap();

    assert!(verdict.will_retry());
    assert_eq!(
        store.get_task(&ids[0]).unwrap().unwrap().state,
        TaskState::RetryWait
    );
}
...
fn retry_becomes_claimable_only_after_the_backoff() {
    ...
    store
        ...
        .unwrap();
    let run = store
        .start_run(&ids[0], "rev", "hash", clock.now())
        .unwrap();
...
fn exhausted_attempts_are_dead_lettered() {
    ...
    assert!(queue
        .record_failure(&mut store, id, "fail 1")
        .unwrap()
        .will_retry());
...
fn dead_letter_reason_records_why() {
    ...
    queue
        .record_failure(&mut store, id, "verifier crashed")
        .unwrap();
...
fn recovery_profile_serialises_work() {
    ...
    assert!(queue.admits_more_work(0));
    assert!(
        !queue.admits_more_work(1),
        "recovery runs one task at a time"
    );
}
