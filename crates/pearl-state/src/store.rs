...
///
/// `schema_migrations` is deliberately absent: it describes the *shape* of these tables
/// rather than their contents, so clearing it would make a rebuild forget which schema it
/// was rebuilding into.
...
impl StateStore {
    ...
    fn init(mut ledger: EventLedger) -> Result<Self, StateError> {
        pearl_events::migrations::apply(
            ledger.connection_mut(),
            crate::migrations::COMPONENT,
            crate::migrations::MIGRATIONS,
        )?;
        Ok(Self { ledger })
    }

    /// The projection schema version this store is at.
    ///
    /// Worth surfacing rather than keeping private: "which schema is this database on" is the
    /// first question when a query fails on a machine you cannot see.
    pub fn schema_version(&self) -> Result<u32, StateError> {
        Ok(pearl_events::migrations::current_version(
...
pub enum StateError {
    ...
    /// The projections could not be brought to the schema this build expects.
    #[error(transparent)]
    Migration(#[from] pearl_events::MigrationError),
